...
! divide by 10 to convert millibar to kpa
          satvapor = sat_vapor_press(Tavgc(i)) / 10.0D0

!...SLOPE OF SATURATION VAPOR PRESSURE CURVE AT AVG TEMP, KPA/DEG(CELSIUS), Irmak 2012, eqn 18
          Vp_slope(i) = (4098.0D0*satvapor)/((Tavgc(i)+237.3D0)**2.0D0) ! delta
...
! Dew point temperature (Lawrence(2005) eqn. 8), degrees C
          ...
          num = B1 * ( DBLE( (LOG(Humidity_hru(i)/100.0)) ) + t1 )
          den = A1 - DBLE(LOG(Humidity_hru(i)/100.0)) - t1
...
! 1 Langley = 0.04184 MJ/m2
          net_rad = Swrad(i)*0.04184D0 - Lwrad_net(i) - heat_flux
...
!  eeq is in cm
          eeq = ratio*net_rad/elh * 100.0D0 ! if eeq<0, the Potet will be set to 0.0 below
