...
! Get parameters
        ...
        IF ( getparam(MODNAME, 'ppt_rad_adj', Nhru_nmonths, 'real', Ppt_rad_adj)/=0 ) CALL read_error(2, 'ppt_rad_adj')
        ...
        Observed_flag = OFF
        IF ( Nsol > 0 ) THEN
          IF ( getparam(MODNAME, 'rad_conv', 1, 'real', Rad_conv)/=0 ) CALL read_error(2, 'rad_conv')
