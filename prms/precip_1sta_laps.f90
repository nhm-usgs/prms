...
!***********************************************************************
      MODULE PRMS_PRECIP_1STA_LAPS
        ...
        DOUBLE PRECISION, SAVE, ALLOCATABLE :: Precip_local(:)
        ! Declared Parameters
     ...
     &    Print_debug, Glacier_flag, Nowmonth, Nhru_nmonths
      ...
      USE PRMS_BASIN, ONLY: Active_hrus, Hru_area_dble, Hru_route_order, Basin_area_inv, Hru_elev_ts, Hru_type
      ...
      IF ( Process_flag==RUN ) THEN
        ...
        DO i = 1, Nrain
          IF ( Psta_nuse(i)==ACTIVE ) THEN
            IF ( Precip_local(i)<0.0D0 ) THEN
              IF ( Print_debug>DEBUG_less ) THEN
                PRINT 9002, Precip_local(i), MODNAME, i
                CALL print_date(1)
              ENDIF
              Precip_local(i) = 0.0D0
