...
! Potential ET Variables
     ...
     &     'inches', Basin_potet)/=0 ) CALL read_error(3, 'basin_potet')
      ...
      IF ( Et_flag==5 .OR. Et_flag==11 .OR. Model==99 ) THEN
      ! For potet_pt and potet_pm
     ...
     &       'kilopascals/degrees Celsius', Vp_slope)/=0 ) CALL read_error(3, 'vp_slope')
        IF ( Et_flag==11 .OR. Model==99 ) THEN
...
! FLOW VARIABLES AND PARAMETERS
      ...
      Basin_potet = 0.0D0
      Potet = 0.0
      IF ( Et_flag==5 .OR. Et_flag==11 ) THEN
        Tempc_dewpt = 0.0
        Vp_actual = 0.0
        Lwrad_net = 0.0
        Vp_slope = 0.0
        IF ( Et_flag==11 ) Vp_sat = 0.0
      ENDIF
      Basin_orad = 0.0D0
...
!***********************************************************************
     ...
     &          Basin_swale_et, Basin_perv_et, Basin_soil_moist, Basin_ssstor, Basin_lakeevap, Basin_lake_stor
        ...
        IF ( Et_flag==5 .OR. Et_flag==11 ) THEN
          ...
          IF ( Et_flag==11 ) WRITE ( Restart_outunit ) Vp_sat
        ENDIF
      ELSE
     ...
     &         Basin_swale_et, Basin_perv_et, Basin_soil_moist, Basin_ssstor, Basin_lakeevap, Basin_lake_stor
        ...
        IF ( Et_flag==5 .OR. Et_flag==11 ) THEN
          ...
          IF ( Et_flag==11 ) READ ( Restart_inunit ) Vp_sat
        ENDIF
      ENDIF
