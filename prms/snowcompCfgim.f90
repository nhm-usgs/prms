...
! PRMS_SNOW module for defining stateful variables
      ...
      INTEGER, SAVE :: Active_glacier, Active_freezing
...
! Frozen ground variables
      REAL, SAVE, ALLOCATABLE :: Tcal_nosnow(:), Land_albedo(:)
! Glacier variables
...
! Glacier declares
     ...
     &       'decimal fraction')/=0 ) CALL read_error(1, 'glrette_frac_init')
      ...
      IF ( Frozen_flag==1 .OR. Model==99 ) THEN
        ALLOCATE ( Tcal_nosnow(Nhru) )
        IF ( declvar(MODNAME, 'tcal_nosnow', 'nhru', Nhru, 'real', &
     &     'Net energy balance on each HRU without snow or glacier', &
     &     'Langleys', Tcal_nosnow)/=0 ) CALL read_error(3, 'tcal_nosnow')
...
! Do for every time step
        ...
        Ai(i) = 0.0D0
        Tcal(i) = 0.0
        IF (Frozen_flag==1) Tcal_nosnow(i) = 0.0
...
!Paterson 2010 says 12 mm/yr for friction and geothermal heating
     ...
     &                  Int_alb(i), Salb(i), Slst(i))
        ...
        IF ( Active_glacier==1 ) Glacrcov_area(i) =(1.0-Snowcov_area(i))*Glacier_frac(i)
        IF ( Active_glacier==2 ) Glacrcov_area(i) =(1.0-Snowcov_area(i))*Glrette_frac(i)
        IF ( Snowcov_area(i)==1.0 .OR. Glacrcov_area(i)==1.0) Active_frozen=0 !no need to separately calculate energy if snow or glacier
...
!   Assumes canopy is assumed to be a perfect blackbody so only want albedo of land under canopy
  ...
  !          dpt1 = ((Net_snow(i)*Deninv)+ (Setden*Pss(i))+Pk_depth(i))*Set1 ! [inches]
     ...
     &                   rjunk, rjunk, djunk, &
     &                   djunk, djunk, rjunk, cst, cals, lsw, rjunk,-100)
            ! track total heat flux from both night and day periods
            Tcal_nosnow(i) = cals ! [cal/cm^2] or [Langleys]
          ENDIF
