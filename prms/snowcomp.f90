...
! PRMS_SNOW module for defining stateful variables
      ...
      character(len=*), parameter :: Version_snowcomp = '2023-10-06'
...
! Functions
      ...
      EXTERNAL :: read_error, sca_deplcrv, snow_states_to_zero, glacr_states_to_zero
! Local Variables
      INTEGER :: i, j
      REAL :: x
! Save Variables
...
!***********************************************************************
     ...
     &    INCH2M, FEET2METERS, ACTIVE, OFF, DEBUG_less, DAYS_YR, TINY_SNOWPACK
...
!rsr10  ENDIF
        ...
        IF ( Newsnow(i)==ACTIVE .AND. Pkwater_equiv(i)<TINY_SNOWPACK ) Snowcov_area(i) = 1.0 ! [fraction of area]
     ...
     &                         Pk_def(i), Freeh2o(i), Pk_temp(i), Hru_intcpevap(i))
          ELSEIF ( Pkwater_equiv(i)<0.0D0 ) THEN
            IF ( Print_debug>DEBUG_less ) THEN
              IF ( Pkwater_equiv(i)<-TINY_SNOWPACK ) PRINT *, 'snowpack issue 3, negative pkwater_equiv, &
...
! LAST check to clear out all arrays if packwater is gone
        IF ( Pkwater_equiv(i)<=0.0D0 ) THEN
          IF ( Print_debug>DEBUG_less ) THEN
            IF ( Pkwater_equiv(i)<-TINY_SNOWPACK ) &
...
!***********************************************************************
     ...
     &           Freeh2o_cap, Den_max, Ihru_gl)
      USE PRMS_CONSTANTS, ONLY: CLOSEZERO, INCH2CM, ACTIVE !, TINY_SNOWPACK
      IMPLICIT NONE
! Functions
...
!          IF ( Pkwater_equiv<-TINY_SNOWPACK ) &
...
!***********************************************************************
      ...
      USE PRMS_CONSTANTS, ONLY: CLOSEZERO !, TINY_SNOWPACK
      IMPLICIT NONE
! Functions
      INTRINSIC :: SNGL
! Arguments
...
! in degF, not degC (Anderson 1968).  Which is correct?
      ...
      IF ( qcond<0.0 ) THEN
        ...
        IF ( Pk_temp<0.0 ) THEN
          ...
          Pk_temp = -Pk_def/SNGL(Pkwater_equiv*1.27D0) ! [degrees C] ! rsr, what if pkwater_equiv < TINY_SNOWPACK
        ELSE
     ...
     &                              Freeh2o_cap, Den_max, Ihru_gl)
      ...
      ELSEIF ( ts>=0.0 ) THEN
        ...
        ELSE
          ...
          Pk_temp = -pk_defsub/SNGL(Pkwater_equiv*1.27D0) ! [degrees C] ! rsr, what if pkwater_equiv < TINY_SNOWPACK
        ENDIF
...
!***********************************************************************
     ...
     &                    Pkwater_equiv, Pk_ice, Pk_def, Freeh2o, Pk_temp, Hru_intcpevap)
      USE PRMS_CONSTANTS, ONLY: CLOSEZERO, TINY_SNOWPACK, DEBUG_less, OFF
      USE PRMS_MODULE, ONLY: Print_debug
      USE PRMS_SNOW, ONLY: Active_glacier
      IMPLICIT NONE
! Functions
