...
!***********************************************************************
      MODULE PRMS_POTET_HAMON
        ...
        character(len=*), parameter :: Version_potet = '2024-01-30'
...
! Local Variables
      ...
      REAL, ALLOCATABLE :: Hamon_coef_sngl(:, :)
!***********************************************************************
      potet_hamon = 0
...
! Convert daylength from hours to 12 hour multiple (equal day and night period)
        IF ( Hru_order_flag == ACTIVE ) THEN
          DO i = 1, Nhru
            ...
            vpsat = 6.108D0*EXP(17.26939D0*Tavgc(i)/(Tavgc(i)+237.3D0))
            vdsat = 216.7D0*vpsat/(Tavgc(i)+273.3D0)
            Potet(i) = Hamon_coef(i, Nowmonth)*dyl*dyl*vdsat
...
!******Get parameters
      ELSEIF ( Process_flag==INIT ) THEN
        ALLOCATE ( Hamon_coef_sngl(Nhru,Nmonths) )
        IF ( getparam(MODNAME, 'hamon_coef', Nhru_nmonths, 'real', Hamon_coef_sngl)/=0 ) CALL read_error(2, 'hamon_coef')
