...
!***********************************************************************
     ...
     &    PRMS4_flag, Glacier_flag, AG_flag
      USE PRMS_BASIN
...
! Declared Variables
     ...
     &     'decimal fraction')/=0 ) CALL read_error(1, 'hru_percent_imperv')
      ...
      IF ( AG_flag==ACTIVE .OR. Model==DOCUMENTATION ) THEN
        IF ( declvar(MODNAME, 'basin_ag', 'one', 1, 'double', &
     &       'Basin area-weighted agricultural area', &
     &       'cfs', Basin_ag)/=0 ) CALL read_error(1, 'basin_ag')

        IF ( CALL declvar(MODNAME, 'ag_area', 'nhru', Nhru, 'real', &
