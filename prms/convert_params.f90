...
!***********************************************************************
      IF ( Process_flag==INIT ) THEN
        ...
        IF ( getparam(MODNAME, 'tmax_allsnow', Nhru_nmonths, 'real', Tmax_allsnow)/=0 ) CALL read_error(2, 'tmax_allsnow')
        ...
        IF ( Model_mode(:8)=='CONVERT4' ) THEN
          IF ( getparam(MODNAME, 'tmax_allrain_offset', Nhru_nmonths, 'real', Tmax_allrain_offset)/=0 ) &
...
!          ENDIF
          ...
          ENDDO
        ELSE
          IF ( getparam(MODNAME, 'tmax_allrain', Nhru_nmonths, 'real', Tmax_allrain)/=0 ) CALL read_error(2, 'tmax_allrain')
