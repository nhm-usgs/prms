...
!***********************************************************************
      ...
      USE PRMS_BASIN, ONLY: CFS2CMS_CONV, Basin_area_inv, METERS2FEET
...
! FIX this should be routed, is it?, should be all going in, including the lateral flow
! muskingum has Seg_upstream_inflow that is the hourly upstream, do I need this?
       Seg_inflow(NETOPO(:)%REACHID) = (METERS2FEET**3.)*RCHFLX(iens,:)%UPSBASIN_QR
       !Seg_upstream_inflow(NETOPO(:)%REACHID) = (METERS2FEET**3.)*RCHFLX(iens,:)%UPSBASIN_QR !daily, not hourly, FIX??
       ...
       if (doKWTroute) then
         ...
         ! write routed runoff (m3/s) into cfs
         Seg_outflow(NETOPO(:)%REACHID)= (METERS2FEET**3.)*RCHFLX(iens,:)%REACH_Q
       end if
