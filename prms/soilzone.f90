...
!***********************************************************************
      ...
      IMPLICIT NONE
!   Local Variables
      INTEGER, SAVE :: BALUNT, DBGUNT, Et_type, Pref_flag, First_run, Max_gvrs
      ...
      REAL, SAVE, ALLOCATABLE :: It0_soil_rechr(:), It0_soil_moist(:), It0_pref_flow_stor(:), It0_ssres_stor(:)
      REAL, SAVE, ALLOCATABLE :: It0_gravity_stor_res(:), It0_sroff(:), It0_slow_stor(:), It0_potet(:)
      DOUBLE PRECISION, ALLOCATABLE :: It0_strm_seg_in(:)
      DOUBLE PRECISION, SAVE :: It0_strm_farfield, Basin_gwin, It0_basin_soil_moist, It0_basin_ssstor
!   Declared Variables
      DOUBLE PRECISION, SAVE :: Basin_sz2gw, Basin_ssr2gw, Basin_interflow_max, Basin_sm2gvr_max
      DOUBLE PRECISION, SAVE :: Basin_soil_rechr, Basin_dunnian_gvr, Basin_recharge, Basin_pref_flow_in
      DOUBLE PRECISION, SAVE :: Basin_ssin, Basin_dunnian_pfr, Basin_sm2gvr, Basin_dninterflow
      DOUBLE PRECISION, SAVE :: Basin_dncascadeflow, Basin_dndunnianflow, Basin_capwaterin, Basin_dunnian
      DOUBLE PRECISION, SAVE :: Basin_gvr2pfr, Basin_slowflow, Basin_szfarflow, Basin_pref_stor
      DOUBLE PRECISION, SAVE :: Basin_slstor, Basin_prefflow, Basin_lakeinsz, Basin_lakeprecip
      DOUBLE PRECISION, SAVE :: Basin_cap_infil_max, Basin_cap_up_max, Basin_capillary_wb, Basin_gravity_wb
      DOUBLE PRECISION, SAVE :: Basin_soilzone_wb, Basin_soil_moist_tot, Basin_gvr2sm
      REAL, SAVE, ALLOCATABLE :: Perv_actet(:), Pref_flow_thrsh(:), Soil_moist_tot(:), Soil_moist_frac(:), Recharge(:)
      REAL, SAVE, ALLOCATABLE :: Upslope_interflow(:), Dunnian_flow(:), Upslope_dunnianflow(:), Infil_tot(:)
      REAL, SAVE, ALLOCATABLE :: Pref_flow_stor(:), Pref_flow(:), Lakein_sz(:), Pref_flow_infil(:)
      REAL, SAVE, ALLOCATABLE :: Hru_sz_cascadeflow(:), Swale_actet(:), Pref_flow_max(:), Snow_free(:)
      REAL, SAVE, ALLOCATABLE :: Soil_zone_max(:), Soil_rechr_ratio(:), Cap_waterin(:), Soil_lower(:)
      REAL, SAVE, ALLOCATABLE :: Potet_lower(:), Potet_rechr(:), Soil_lower_ratio(:)
      REAL, SAVE, ALLOCATABLE :: Sm2gw_grav(:), Sm2gw_grav_old(:), Gravity_stor_res(:), Gvr2sm(:)
...
!   Declared Parameters
      ...
      REAL, SAVE, ALLOCATABLE :: Soil_moist_init(:), Ssstor_init(:), Pref_flow_den(:)
      REAL, SAVE, ALLOCATABLE :: Fastcoef_lin(:), Fastcoef_sq(:), Slowcoef_lin(:), Slowcoef_sq(:)
      REAL, SAVE, ALLOCATABLE :: Ssr2gw_rate(:), Ssr2gw_exp(:), Soil_rechr_init(:), Soil2gw_max(:), Lake_evap_adj(:, :)
...
!***********************************************************************
      ...
      USE PRMS_SOILZONE
      USE PRMS_MODULE, ONLY: Model, Nhru, Nssr, Nsegment, Nlake, Nhrucell, Print_debug, Cascade_flag
      USE PRMS_BASIN, ONLY: Timestep
      IMPLICIT NONE
! Functions
...
!***********************************************************************
      ...
      Version_soilzone = '$Id: soilzone.f90 5532 2013-03-25 21:49:54Z rsregan $'
...
! Declare Variables
     ...
     &     'Basin area-weighted average infiltration, cascading interflow and Dunnian flow added to capillary reservoir storage', &
     &     'inches', Basin_capwaterin)/=0 ) CALL read_error(3, 'basin_capwaterin')

      IF ( declvar(MODNAME, 'basin_cap_infil_max', 'one', 1, 'double', &
     &     'Basin area-weighted average maximum infiltration that flows to capillary reservoirs', &
     &     'inches', Basin_cap_infil_max)/=0 ) CALL read_error(3, 'basin_cap_infil_max')

      IF ( declvar(MODNAME, 'basin_cap_up_max', 'one', 1, 'double', &
     &     'Basin area-weighted average maximum cascade flow that flows to the capillary reservoirs', &
     ...
     &     'inches', Basin_pref_flow_in)/=0 ) CALL read_error(3, 'basin_pref_flow_in')
     ...
     &     'Basin area-weighted average storage for recharge zone; upper portion of capillary reservoir where both'// &
     ...
     &     'Basin area-weighted average maximum excess flow from capillary reservoirs that flows to gravity reservoirs', &
     ...
     &       'Basin area-weighted average cascading interflow, Dunnian surface runoff, and farflow', &
     &       'inches', Basin_dncascadeflow)/=0 ) CALL read_error(3, 'basin_dncascadeflow')

        ALLOCATE ( Upslope_interflow(Nhru) )
        IF ( declvar(MODNAME, 'upslope_interflow', 'nhru', Nhru, 'real', &
     &       'Cascading interflow runoff that flows to the capillary reservoir of each down slope HRU'// &
     ...
     &       'real', 'Cascading Dunnian surface runoff that flows to the capillary reservoir of each down'// &
     ...
     &       'Cascading interflow, Dunnian surface runoff, and farflow from each HRU', &
     ...
     &     'Soil storage threshold defining storge between field capacity and maximum soil saturation minus'// &
     ...
     &     'Infiltration and any cascading interflow and Dunnian surface runoff added to capillary'// &
     ...
     &     'Storage in the lower zone of the capillary reservoir that is only available for'// &
     ...
     &       'inches', Gvr2sm)/=0 ) CALL read_error(3, 'gvr2sm')
        ...
        ALLOCATE ( Hru_gvr_count(Nhru), Hrucheck(Nhru), Gvr_hru_id(Nhrucell), Gvr_hru_pct_adjusted(Nhrucell) )
        ALLOCATE ( It0_pref_flow_stor(Nhru), It0_ssres_stor(Nhru), It0_soil_rechr(Nhru), It0_soil_moist(Nhru) )
        ALLOCATE ( It0_gravity_stor_res(Nhrucell), It0_sroff(Nhru), It0_slow_stor(Nhru), Gw2sm_grav(Nhrucell) )
...
! Allocate arrays for parameters, local and variables from other modules
      IF ( Nlake>0 .OR. Model==99 ) ALLOCATE ( Lake_evap_adj(12,Nlake) )
      ALLOCATE ( Soil2gw(Nhru), Gvr2pfr(Nhru), Swale_limit(Nhru) )
      ALLOCATE ( Pref_flow_flag(Nhru), Slowcoef_lin(Nhru), Slowcoef_sq(Nhru), Pref_flow_den(Nhru), Soil2gw_max(Nhru) )
      ALLOCATE ( Soil_type(Nhru), Fastcoef_lin(Nhru), Fastcoef_sq(Nhru), Ssr2gw_rate(Nhru), Ssr2gw_exp(Nhru) )
...
! Declare Parameters
     ...
     &     'Initial storage for soil recharge zone (upper part of capillary reservoir where losses occur as both'// &
     &     ' evaporation and transpiration) for each HRU; must be less than or equal to soil_moist_init', &
     ...
     &     'Non-linear coefficient in equation used to route preferential-flow storage down slope for each HRU', &
     ...
     &     'Non-linear coefficient in equation used to route water from the gravity reservoir to the GWR for each HRU', &
 ...
 9001 FORMAT ('    Date     Water Bal     bsmbal    last SM  soilmoist  last stor    SS stor    perv ET      sz2gw  interflow', &
     &        '    soil2gw    Dunnian    soil in   lakeinsz   downflow    farflow   swale ET  pref flow   pfr dunn   gvr dunn', &
     &        '  lake evap')
 9009 FORMAT ('    Date     Water Bal     bsmbal    last SM  soilmoist  last stor    SS stor    perv ET      sz2gw  interflow', &
     &        '    soil2gw    Dunnian    soil in   lakeinsz   downflow    farflow   swale ET  pref flow   pfr dunn   gvr dunn', &
...
!***********************************************************************
      ...
      USE PRMS_SOILZONE
      USE PRMS_MODULE, ONLY: Print_debug, Nhru, Nssr, Nlake, Model, Nhrucell, Inputerror_flag, Parameter_check_flag, Cascade_flag
      USE PRMS_BASIN, ONLY: Hru_type, Hru_perv, Hru_route_order, Basin_area_inv, Hru_area, NEARZERO, Hru_frac_perv, Active_hrus
      USE PRMS_FLOWVARS, ONLY: Soil_moist_max, Soil_rechr_max, Ssres_stor, Basin_ssstor, Basin_soil_moist, Slow_stor, &
...
!***********************************************************************
      ...
      DO j = 1, Active_hrus
        ...
        IF ( Model==0 ) THEN
          IF ( Soil_moist_max(i)>NEARZERO ) Replenish_frac(i) = Soil_rechr_max(i)/Soil_moist_max(i)
        ENDIF
        IF ( Hru_type(i)==3 ) THEN ! swales
          IF ( Pref_flow_den(i)>0.0 ) THEN
            IF ( Parameter_check_flag==1 ) THEN
              PRINT *, 'ERROR, pref_flow_den must be 0 for swale HRU:', i, Pref_flow_den(i)
              ierr = 1
              CYCLE
            ENDIF
            IF ( Print_debug>-1 ) THEN
              PRINT *, 'Warning, pref_flow_den must be 0 for swale HRUs'
              PRINT *, 'reset from:', Pref_flow_den(i), ' to 0.0 for HRU:', i
            ENDIF
          ENDIF
          Pref_flow_den(i) = 0.0
...
! initialize arrays (dimensioned Nhrucell)
      ...
      IF ( Model==0 ) THEN
        Hru_gvr_count = 0
        Hrucheck = 0 !if inactive or lake
        DO j = 1, Active_hrus
          i = Hru_route_order(j)
          IF ( Hru_type(i)/=0 .AND. Hru_type(i)/=2 ) Hrucheck(i) = 1
        ENDDO
        DO i = 1, Nhrucell
          ii = Gvr_hru_id(i)
          IF ( ii>Nhru ) THEN
          ...
          Gravity_stor_res(i) = Ssstor_init(ii)
      ...
      DO ii = 1, Active_hrus
        ...
        IF ( Pref_flow_flag(i)==1 ) THEN
          ...
          Basin_pref_stor = Basin_pref_stor + Pref_flow_stor(i)*Hru_area(i)
        ENDIF
        Soil_moist_tot(i) = Ssres_stor(i) + Soil_moist(i)*Hru_frac_perv(i)
        Basin_soil_moist_tot = Basin_soil_moist_tot + Soil_moist_tot(i)*Hru_area(i)
 ...
 9002 FORMAT (/, 'ERROR, HRU:', I6, F10.4, F8.4, ' soil_rechr_max > soil_moist_max')
 9003 FORMAT (/, 'ERROR, HRU:', I6, F10.4, F8.4, ' soil_rechr_init > soil_rechr_max')
 9004 FORMAT (/, 'ERROR, HRU:', I6, F10.4, F8.4, ' soil_moist_init > soil_moist_max')
 9005 FORMAT (/, 'ERROR, HRU:', I6, F10.4, F8.4, ' soil_rechr > soil_moist based on init and max values')
 9006 FORMAT (/, 'ERROR, HRU:', I6, A, ' value < 0.0', F10.4)
 9008 FORMAT (/, 'ERROR, HRU:', I6, A, ' value invalid:', I6)
 9012 FORMAT ('Warning, HRU:', I6, F10.4, F8.4, '  soil_rechr_max > soil_moist_max,', /, 29X, &
     &        'soil_rechr_max set to soil_moist_max')
 9013 FORMAT ('Warning, HRU:', I6, F10.4, F8.4, ' soil_rechr_init > soil_rechr_max,', /, 29X, &
     &        'soil_rechr set to soil_rechr_max')
 9014 FORMAT ('Warning, HRU:', I6, F10.4, F8.4, '  soil_moist_init > soil_moist_max,', /, 29X, &
     &        'soil_moist set to soil_moist_max')
 9015 FORMAT ('Warning, HRU:', I6, F10.4, F8.4, ' soil_rechr_init > soil_moist_init,', /, 29X, &
...
!***********************************************************************
      ...
      USE PRMS_SOILZONE
      USE PRMS_MODULE, ONLY: Nhru, Nssr, Dprst_flag, Print_debug, Nsegment, Model, Nlake, Nhrucell, Kkiter, Cascade_flag
      USE PRMS_BASIN, ONLY: Hru_type, Hru_perv, Hru_frac_perv, Hru_route_order, Active_hrus, Basin_area_inv, Hru_area, &
...
! WARNING!!! Sroff, Basin_sroff, and Strm_seg_in can be updated
      USE PRMS_FLOWVARS, ONLY: Basin_ssflow, Basin_actet, Hru_actet, Ssres_flow, Soil_to_gw, Basin_soil_to_gw, Ssr_to_gw, &
     &    Soil_to_ssr, Basin_lakeevap, Basin_perv_et, Basin_swale_et, Sroff, Soil_moist_max, Infil, &
     &    Soil_rechr_max, Ssres_in, Basin_soil_moist, Basin_ssstor, Slow_stor, Slow_flow, &
     &    Ssres_stor, Soil_moist, Sat_threshold, Soil_rechr
      ...
      USE PRMS_SRUNOFF, ONLY: Basin_sroff, Basin_infil, Hru_impervevap, Strm_seg_in, Strm_farfield, Dprst_evap_hru
      IMPLICIT NONE
! Functions
      INTRINSIC MIN, ABS, MAX
      INTEGER, EXTERNAL :: getvar
      EXTERNAL compute_soilmoist, compute_szactet, compute_cascades
      EXTERNAL compute_interflow, comp_inter_gwflow, read_error, compute_gravflow, init_basin_vars
! Local Variables
      INTEGER :: i, k, ncasc, ihru
      DOUBLE PRECISION :: bsmbal, waterin, gwin, basin_dunnian_pfr_only, soilbal, basin_bal, test, soil_in, gvrbal
      REAL :: dunnianflw, last_sm, interflow, last_ss, perv_area, harea, dnslowflow, dnpreflow, dndunn, availh2o, avail_potet
      REAL :: farflow_slow, farflow_pref, farflow_dunn, tmp, gvr_maxin, dunnianflw_pfr, dunnianflw_gvr, pref_flow_maxin, frac
      REAL :: hru_cascadeflow, perv_frac, capacity, capwater_maxin, cap_upflow_max, extra_water, pref_flow_in, gvr2sm_val
!***********************************************************************
      szrun = 0
      ...
      IF ( Model==0 ) THEN
        ...
        IF ( First_run==1 ) THEN
          IF ( getvar(MODNAME, 'gvr_hru_pct_adjusted', Nhrucell, 'double', Gvr_hru_pct_adjusted)/=0 ) &
...
! use value from last iteration for each time step instead of 0.0
          IF ( getvar(MODNAME, 'gw2sm_grav', Nhrucell, 'real', Gw2sm_grav)/=0 ) CALL read_error(4, 'gw2sm_grav')
        ENDIF
      ENDIF
...
!****** add soil excess (Dunnian flow) to infiltration
          ...
          IF ( capwater_maxin>0.0 ) THEN
            IF ( Pref_flow_flag(i)==1 ) THEN
              ...
              Basin_pref_flow_in = Basin_pref_flow_in + Pref_flow_infil(i)*harea
            ENDIF
            Basin_cap_infil_max = Basin_cap_infil_max + capwater_maxin*perv_area
          ENDIF
...
!******Add infiltration to soil and compute excess
          ...
          IF ( capwater_maxin>0.0 ) THEN
            ...
            CALL compute_soilmoist(Cap_waterin(i), Soil_moist_max(i),  Soil_rechr_max(i), Soil2gw_max(i), gvr_maxin, &
...
! compute slow_flow and ssr_to_gw
          IF ( Model==0 ) THEN
            ...
            CALL compute_gravflow(i, capacity, Slowcoef_lin(i), Slowcoef_sq(i), Ssr2gw_rate(i), Ssr2gw_exp(i), &
     &           gvr_maxin, Pref_flow_thrsh(i), Soil_to_ssr(i), Gvr2pfr(i), Ssr_to_gw(i), Slow_flow(i), Slow_stor(i), &
...
!              IF ( Soil_moist(i)>Soil_moist_max(i) ) PRINT *, 'sm>max', Soil_moist(i), Soil_moist_max(i),i
          ...
          ELSE
            ...
            IF ( Slow_stor(i)>0.0 ) CALL comp_inter_gwflow(Slowcoef_lin(i), Slowcoef_sq(i), &
     &                Ssr2gw_rate(i), Ssr2gw_exp(i), Pref_flow_thrsh(i), Gvr2pfr(i), Ssr_to_gw(i), Slow_flow(i), &
     &                Slow_stor(i), Hru_type(i), Soil_to_ssr(i))
          ...
          IF ( Pref_flow_flag(i)==1 ) THEN
            ...
            IF ( extra_water>0.0 ) THEN
              ...
              IF ( Gvr2pfr(i)<0.0 ) THEN
!                PRINT *, 'gvr2pfr<0', Gvr2pfr(i), extra_water, pref_flow_max(i), pref_flow_stor(i), gvr_maxin
                Gvr2pfr(i) = 0.0
              ENDIF
              Pref_flow_stor(i) = Pref_flow_max(i)
     ...
     &           CALL compute_interflow(Fastcoef_lin(i), Fastcoef_sq(i), pref_flow_in, Pref_flow_stor(i), Pref_flow(i))
          ELSE
            dunnianflw_gvr = Gvr2pfr(i)
            Gvr2pfr(i) = 0.0
          ENDIF
...
!******Compute actual evapotranspiration
          ...
          IF ( Soil_moist(i)>0.0 ) THEN
            CALL compute_szactet(Soil_moist_max(i), Soil_rechr_max(i), Snowcov_area(i), Transp_on(i), Cov_type(i), &
     &           Soil_type(i), Soil_moist(i), Soil_rechr(i), Perv_actet(i), avail_potet, Snow_free(i), &
     &           Potet_rechr(i), Potet_lower(i), Soil_rechr_ratio(i), Soil_lower_ratio(i))
            ! sanity check
...
!              PRINT *, 'perv_et problem', Perv_actet, Avail_potet
          ...
          IF ( avail_potet<0.0 ) THEN
            IF ( Print_debug>-1 ) THEN
              IF ( avail_potet<-NEARZERO ) PRINT *, 'hru_actet>potet', i, Nowmonth, Nowday, Hru_actet(i), Potet(i), avail_potet
            ENDIF
            Hru_actet(i) = Potet(i)
...
! compute interflow and excess flow to each HRU or stream
            ...
            IF ( Cascade_flag==1 ) THEN
              ...
              IF ( ncasc>0 ) THEN
                IF ( interflow+dunnianflw>0.0 ) THEN
                  CALL compute_cascades(i, ncasc, Slow_flow(i), Pref_flow(i), Dunnian_flow(i), dnslowflow, &
     &                 dnpreflow, dndunn, farflow_slow, farflow_pref, farflow_dunn)
                  ...
                  Basin_dninterflow = Basin_dninterflow + (dnslowflow+dnpreflow)*harea
                  Basin_dndunnianflow = Basin_dndunnianflow + dndunn*harea
                  Basin_szfarflow = Basin_szfarflow + (farflow_slow+farflow_pref+farflow_dunn)*harea
                  Basin_dncascadeflow = Basin_dncascadeflow + (dnslowflow+dnpreflow+dndunn)*harea &
     &                                  + (farflow_slow+farflow_pref+farflow_dunn)*harea
                ELSE
                  Hru_sz_cascadeflow(i) = 0.0
                ENDIF
                basin_dunnian_pfr_only = basin_dunnian_pfr_only + dunnian_flow(i)*frac
              ENDIF
            ENDIF
...
!      and model domain, thus not included in basin_cfs
            ...
            IF ( availh2o>0.0 ) THEN
              ...
              IF ( Print_debug==7 ) THEN
                IF ( Slow_stor(i)>Swale_limit(i) ) THEN
                  WRITE ( DBGUNT, * ) 'Swale ponding, HRU:', i, ' gravity reservoir is 3*sat_threshold', &
     &                    Slow_stor(i), Sat_threshold(i), Nowtime
          ...
          Basin_soil_moist_tot = Basin_soil_moist_tot + Soil_moist_tot(i)*harea
          Soil_moist_frac(i) = Soil_moist_tot(i)/Soil_zone_max(i)

          IF ( Print_debug==1 ) THEN
            gvr2sm_val = 0.0
            IF ( Model==0 ) gvr2sm_val = Gvr2sm(i)
            soilbal = (capwater_maxin + last_sm-Soil_moist(i) - Perv_actet(i))*perv_frac - gvr_maxin &
     &                - Soil_to_gw(i) + gvr2sm_val
            IF ( ABS(soilbal)>TOOSMALL ) THEN
              WRITE ( BALUNT, * ) 'HRU capillary problem'
              WRITE ( BALUNT, * ) soilbal, Infil_tot(i), last_sm, Soil_moist(i), Perv_actet(i), Soil_to_ssr(i), &
     &                Soil_to_gw(i), i, Infil(i), Pref_flow_infil(i), perv_frac, capwater_maxin-Infil_tot(i), &
     &                capwater_maxin, gvr2sm_val, Soil_moist_max(i), Cap_waterin(i), gvr_maxin
              IF ( Cascade_flag==1 ) WRITE ( BALUNT, * ) 'UP', Upslope_interflow(i), Upslope_dunnianflow(i)
            ENDIF
            gvrbal = last_ss - Ssres_stor(i) + gvr_maxin - Ssr_to_gw(i) - Swale_actet(i) - Dunnian_flow(i) - &
     &               hru_cascadeflow - Ssres_flow(i) - gvr2sm_val + dunnianflw_pfr + gwin + Pref_flow_infil(i)
            test = ABS( gvrbal )
            IF ( test>TOOSMALL ) THEN
              WRITE ( BALUNT, * ) 'Bad GVR balance, HRU:', i, ' hru_type:', Hru_type(i)
              WRITE ( BALUNT, * ) gvrbal, last_ss, Ssres_stor(i), gvr_maxin, Ssr_to_gw(i), Swale_actet(i), &
     &                Dunnian_flow(i), hru_cascadeflow, Ssres_flow(i), gvr2sm_val, dunnianflw_pfr, gwin, &
     &                Pref_flow_infil(i), dunnianflw_gvr, dnslowflow, dnpreflow, farflow_slow, farflow_pref, &
     &                farflow_dunn, Slow_flow(i), Pref_flow(i), dndunn, dunnianflw, Soil_to_ssr(i), interflow, Gvr2pfr(i), &
     &                perv_frac, Slow_stor(i), Pref_flow_stor(i), Infil(i), Pref_flow_max(i), Pref_flow_den(i), &
     &                Pref_flow_thrsh(i), Ssres_in(i)
            ENDIF

            waterin = capwater_maxin*perv_frac + Pref_flow_infil(i) + gwin + dunnianflw_pfr
            soil_in = soil_in + (Infil(i)*perv_frac + gwin)*harea
            soilbal = waterin + last_ss - Ssres_stor(i) + (last_sm-Soil_moist(i)-Perv_actet(i))*perv_frac - &
     &                Ssr_to_gw(i) - interflow - dunnianflw - Soil_to_gw(i) - Swale_actet(i)
...
!              IF ( test>Ssres_stor(i)*TOOSMALL ) THEN
                ...
                ELSE
                  WRITE ( BALUNT, * ) 'possible HRU water balance rounding issue'
                ENDIF
                WRITE ( BALUNT, 9001 ) Nowyear, Nowmonth, Nowday, i, soilbal, Infil(i), last_sm, last_ss, &
     &                Soil_moist(i), Ssres_stor(i), Perv_actet(i), Ssr_to_gw(i), interflow, Slow_flow(i), Pref_flow(i), &
     &                dunnianflw, Soil_to_gw(i), Pref_flow_infil(i), Pref_flow_stor(i), Slow_stor(i), Soil_rechr(i), &
     &                Soil_lower(i), Soil_to_ssr(i), Ssres_flow(i), waterin, Swale_actet(i), gwin, gvr2sm_val
                IF ( Cascade_flag==1 ) WRITE ( BALUNT, * ) 'upslope', Upslope_dunnianflow(i), Upslope_interflow(i)
                WRITE ( BALUNT, * ) perv_area, perv_frac, ncasc, Pref_flow_den(i)
!              ENDIF
            ENDIF
          ENDIF
        ...
        ELSE ! else it is a lake or reservoir
          ...
          IF ( Lake_hru_id(i)>0 ) Hru_actet(i) = Hru_actet(i)*Lake_evap_adj(Nowmonth,Lake_hru_id(i))
...
!            PRINT *, 'Warning, lake evap > potet, for HRU:', i, ' potential ET increased to adjusted lake ET'
      ...
      Basin_capillary_wb = Last_soil_moist - Basin_soil_moist - Basin_perv_et - Basin_sm2gvr_max + Basin_cap_infil_max + &
     &                     Basin_gvr2sm - Basin_soil_to_gw + Basin_cap_up_max
      Basin_gravity_wb = Last_ssstor - Basin_ssstor + Basin_sm2gvr_max - Basin_interflow_max - Basin_sz2gw - Basin_dunnian_gvr - &
     &                   Basin_swale_et - Basin_gvr2sm + Basin_pref_flow_in + Basin_gwin
      Basin_soilzone_wb = Basin_infil + Basin_gwin + Last_ssstor - Basin_ssstor + Last_soil_moist - Basin_soil_moist - &
     &                    Basin_perv_et - Basin_swale_et - Basin_sz2gw - Basin_soil_to_gw - Basin_ssflow - Basin_dunnian - &
     &                    Basin_szfarflow - Basin_lakeinsz

      IF ( Print_debug==1 ) THEN
        IF ( ABS(Basin_gravity_wb)>2.0D-5 ) WRITE ( BALUNT, * ) 'basin gvrbal issue', Basin_gravity_wb, &
     &       Last_ssstor, Basin_ssstor, Basin_sm2gvr_max, Basin_interflow_max, Basin_sz2gw, Basin_dunnian_gvr, &
     &       Basin_swale_et, Basin_gvr2sm, Basin_pref_flow_in, Basin_dninterflow, Basin_pref_stor, Basin_szfarflow, &
     &       Basin_dunnian_pfr, Basin_lakeinsz, Basin_dncascadeflow, Kkiter
        IF ( ABS(Basin_capillary_wb)>TOOSMALL ) WRITE( BALUNT, * ) 'possible basin capillary balance issue', &
     &       Basin_capillary_wb, Last_soil_moist, Basin_soil_moist, Basin_perv_et, Basin_sm2gvr_max, &
     &       Basin_cap_infil_max, Basin_gvr2sm, Basin_soil_to_gw, Basin_cap_up_max, Basin_capwaterin, Basin_soil_to_gw
        IF ( ABS(Basin_soilzone_wb)>0.0005D0 ) WRITE ( BALUNT, * ) 'possible basin soil zone rounding issue', &
     &            Basin_soilzone_wb, Basin_capwaterin, Basin_pref_flow_in, Basin_infil, Basin_gwin, &
     ...
     &            Basin_gvr2sm, Basin_sm2gvr, Basin_lakeinsz, Basin_interflow_max, basin_dunnian_pfr_only
     ...
     &           Basin_ssflow - Basin_soil_to_gw - Basin_dunnian - Basin_szfarflow - Basin_swale_et - Basin_lakeinsz
        ...
        IF ( Model==1 ) THEN
          WRITE ( BALUNT, 9002 ) Nowyear, Nowmonth, Nowday, basin_bal, bsmbal, Last_soil_moist, Basin_soil_moist, Last_ssstor, &
     &            Basin_ssstor, Basin_perv_et, Basin_sz2gw, Basin_ssflow, Basin_soil_to_gw, Basin_dunnian, soil_in, &
     &            Basin_lakeinsz, Basin_dncascadeflow, Basin_szfarflow, Basin_swale_et, Basin_prefflow, Basin_dunnian_pfr, &
     &            Basin_dunnian_gvr, Basin_lakeevap
        ELSE
          WRITE ( BALUNT, 9002 ) Nowyear, Nowmonth, Nowday, basin_bal, bsmbal, Last_soil_moist, Basin_soil_moist, Last_ssstor, &
     &            Basin_ssstor, Basin_perv_et, Basin_sz2gw, Basin_ssflow, Basin_soil_to_gw, Basin_dunnian, soil_in,Basin_lakeinsz, &
     &            Basin_dncascadeflow, Basin_szfarflow, Basin_swale_et, Basin_prefflow, Basin_dunnian_pfr, Basin_dunnian_gvr, &
     &            Basin_lakeevap, Basin_gvr2sm, Basin_gwin, Kkiter
        ENDIF

        IF ( ABS(bsmbal)>0.05D0 .OR. ABS(basin_bal)>0.001D0 ) THEN
          WRITE ( BALUNT, * ) '*ERROR, basin water balance', bsmbal, basin_bal, Last_soil_moist, Basin_soil_moist, &
     &            Last_ssstor, Basin_ssstor, Basin_perv_et, Basin_sz2gw, soil_in, Basin_ssflow, Basin_soil_to_gw,Basin_dunnian, &
     &            Basin_szfarflow, Basin_swale_et, Basin_lakeinsz
          WRITE ( BALUNT, * ) Basin_pref_stor, Basin_slstor
        ELSEIF ( ABS(bsmbal)>0.005D0 .OR. ABS(basin_bal)>TOOSMALL ) THEN
          WRITE ( BALUNT, * ) 'Possible basin water balance ERROR', bsmbal, basin_bal, Last_soil_moist, Basin_soil_moist, &
     &            Last_ssstor, Basin_ssstor, Basin_perv_et, Basin_sz2gw, soil_in, Basin_ssflow, Basin_soil_to_gw,Basin_dunnian, &
     &            Basin_szfarflow, Basin_swale_et, Basin_lakeinsz
          WRITE ( BALUNT, * ) Basin_pref_stor, Basin_slstor
        ELSEIF ( ABS(bsmbal)>0.0005D0 .OR. ABS(basin_bal)>TOOSMALL ) THEN
          WRITE ( BALUNT, '(A,2F12.7)' ) 'Basin rounding issue', bsmbal, basin_bal
          WRITE ( BALUNT, * ) Basin_soilzone_wb, Basin_ssin, Basin_dninterflow, Basin_sm2gvr, Basin_capwaterin, &
     &            soil_in, Basin_gvr2pfr, Basin_gvr2sm, Basin_dndunnianflow, (soil_in - Basin_infil - Basin_gwin)
        ENDIF
      ENDIF
...
!     Soil_to_ssr for whole HRU
!***********************************************************************
      SUBROUTINE compute_soilmoist(Infil, Soil_moist_max, Soil_rechr_max, Soil2gw_max, Soil_to_ssr, Soil_moist, &
...
!     Compute actual evapotranspiration
!***********************************************************************
      SUBROUTINE compute_szactet(Soil_moist_max, Soil_rechr_max, Snowcov_area, Transp_on, Cov_type, Soil_type, &
     &           Soil_moist, Soil_rechr, Perv_actet, Avail_potet, Snow_free, Potet_rechr, Potet_lower, Soil_rechr_ratio, &
     &           Soil_lower_ratio)
...
! Arguments
      ...
      REAL, INTENT(IN) :: Soil_moist_max, Soil_rechr_max, Snowcov_area
      REAL, INTENT(INOUT) :: Soil_moist, Soil_rechr, Avail_potet, Snow_free
      REAL, INTENT(OUT) :: Perv_actet, Potet_rechr, Potet_lower, Soil_lower_ratio, Soil_rechr_ratio
! Local Variables
...
!******sandy soil
        IF ( Soil_type==1 ) THEN
          IF ( Soil_lower_ratio<0.25 ) Potet_lower = 0.5*Soil_lower_ratio*Avail_potet
          IF ( Soil_rechr_ratio<0.25 ) Potet_rechr = 0.5*Soil_rechr_ratio*Avail_potet
!******loam soil
        ELSEIF ( Soil_type==2 ) THEN
          IF ( Soil_lower_ratio<0.5 ) Potet_lower = Soil_lower_ratio*Avail_potet
          IF ( Soil_rechr_ratio<0.5 ) Potet_rechr = Soil_rechr_ratio*Avail_potet
!******clay soil
        ELSEIF ( Soil_type==3 ) THEN
          IF ( Soil_lower_ratio<TWOTHIRDS .AND. Soil_lower_ratio>ONETHIRD ) THEN
          ...
          ENDIF
          IF ( Soil_rechr_ratio<TWOTHIRDS .AND. Soil_rechr_ratio>ONETHIRD ) THEN
...
!***********************************************************************
      SUBROUTINE comp_inter_gwflow(Slowcoef_lin, Slowcoef_sq, Ssr2gw_rate, Ssr2gw_exp, Pref_flow_thrsh, &
     &           Gvr2pfr, Ssr_to_gw, Slow_flow, Slow_stor, Hru_type, Soil_to_ssr)
      USE PRMS_BASIN, ONLY: NEARZERO
! Functions
      IMPLICIT NONE
      INTRINSIC MAX
...
! Arguments
      ...
      REAL, INTENT(IN) :: Slowcoef_lin, Slowcoef_sq, Ssr2gw_rate, Ssr2gw_exp, Pref_flow_thrsh
      REAL, INTENT(INOUT) :: Slow_stor, Gvr2pfr, Ssr_to_gw, Slow_flow, Soil_to_ssr
! Local Variables
      REAL :: extra_water
!***********************************************************************
      IF ( Hru_type==1 ) THEN
        ...
        IF ( Slow_stor>0.0 ) CALL compute_interflow(Slowcoef_lin, Slowcoef_sq, Soil_to_ssr, Slow_stor, Slow_flow)
      ENDIF
...
! sanity check
      IF ( Inter_flow<0.0 ) THEN
        IF ( Inter_flow<-1.0E-6 ) PRINT *, 'interflow<0', Inter_flow, Ssres_in, Storage
      ...
      Storage = Storage - Inter_flow
      IF ( Storage<0.0 ) THEN
        IF ( Print_debug==7 ) WRITE ( DBGUNT, * ) 'Sanity check, ssres_stor<0.0', Storage
...
!***********************************************************************
      SUBROUTINE check_gravity(Pref_flow_thrsh, Depth, Gvr2pfr, Input, Availh2o)
...
!***********************************************************************
      SUBROUTINE compute_cascades(Ihru, Ncascade_hru, Slowflow, Preflow, Dunnian, Dnslowflow, Dnpreflow, Dndunnflow, &
...
! Arguments
      ...
      REAL, INTENT(INOUT) :: Dunnian, Slowflow, Preflow, Dnslowflow, Dnpreflow, Dndunnflow
...
! if hru_down(k, Ihru) > 0, cascade contributes to a downslope HRU
        IF ( j>0 ) THEN
          ...
          Upslope_interflow(j) = Upslope_interflow(j) + (Slowflow+Preflow)*fracwt
          Upslope_dunnianflow(j) = Upslope_dunnianflow(j) + Dunnian*fracwt
...
! if hru_down(k, Ihru) < 0, cascade contributes to a stream
        ELSEIF ( j<0 ) THEN
          ...
          IF ( j/=Nsegmentp1 ) THEN
            Strm_seg_in(j) = Strm_seg_in(j) + (Slowflow+Preflow+Dunnian)*Cascade_area(k, Ihru)*Cfs_conv
          ELSE
            Strm_farfield = Strm_farfield + (Slowflow+Preflow+Dunnian)*Cascade_area(k, Ihru)*Cfs_conv
...
!     compute interflow and flow to groundwater reservoir
!***********************************************************************
      SUBROUTINE compute_gravflow(Ihru, Capacity, Slowcoef_lin, Slowcoef_sq, Ssr2gw_rate, Ssr2gw_exp, Gvr_maxin, &
     &           Pref_flow_thrsh, Soil_to_ssr, Gvr2pfr, Ssr_to_gw, Slow_flow, Slow_stor, Gvr2sm, Soil_to_gw, Gwin, &
     &           Perv_frac, Hru_type)
      USE PRMS_SOILZONE, ONLY: Gvr_hru_id, Gvr_hru_pct_adjusted, Gravity_stor_res, Sm2gw_grav, Sm2gw_grav_old, &
...
! Arguments
      ...
      REAL, INTENT(IN) :: Capacity, Slowcoef_lin, Slowcoef_sq, Perv_frac, Ssr2gw_rate, Ssr2gw_exp
...
! Local Variables
      INTEGER :: j, igvr
      REAL :: perc, slowflow, extra_water, actual_input, depth, input, hru_capacity
      DOUBLE PRECISION :: frac, dunnianflw, topfr, slflow, togw, slowstor
!***********************************************************************
...
! in init, set an array dimensioned by nhrucell to vks*mfl_to_inch
      ...
      DO j = 1, Hru_gvr_count(Ihru)
        ...
        IF ( depth>0.0 .AND. hru_capacity>0.0 ) CALL check_gvr_sm(hru_capacity, depth, frac, Gvr2sm, input)
...
! compute slow contribution to interflow, if any
          IF ( depth>0.0 ) THEN
            CALL compute_interflow(Slowcoef_lin, Slowcoef_sq, actual_input, depth, slowflow)
...
! add any direct recharge from soil infiltration
      ...
      Ssr_to_gw = togw
      Slow_stor = slowstor
      IF ( Slow_stor>Pref_flow_thrsh ) PRINT *, 'slow_stor > thrsh', Slow_stor, Pref_flow_thrsh
...
! Arguments
      ...
      REAL, INTENT(INOUT) :: Capacity, Gvr2sm, Input, Depth
! Local Variables
      REAL :: to_sm
!***********************************************************************
...
!***********************************************************************
      ...
      USE PRMS_SOILZONE
      USE PRMS_FLOWVARS, ONLY: Basin_soil_moist, Basin_ssstor, Basin_actet, Basin_perv_et, &
...
!***********************************************************************
      ...
      USE PRMS_MODULE, ONLY: Restart_outunit, Restart_inunit, Nlake, Model, Cascade_flag, Nhru
      USE PRMS_SOILZONE
      IMPLICIT NONE
      ! Argument
...
!***********************************************************************
      IF ( In_out==0 ) THEN
        ...
        WRITE ( Restart_outunit ) Pref_flag, Basin_gwin, Basin_sz2gw, Basin_ssr2gw, Basin_interflow_max, Basin_sm2gvr_max, &
     &          Basin_soil_rechr, Basin_dunnian_gvr, Basin_recharge, Basin_ssin, Basin_sm2gvr, Basin_capwaterin, Basin_dunnian, &
     &          Basin_slowflow, Basin_slstor, Basin_cap_infil_max, Basin_cap_up_max, Basin_capillary_wb, &
     &          Basin_gravity_wb, Basin_soilzone_wb, Basin_soil_moist_tot, Basin_gvr2sm, Max_gvrs
        WRITE ( Restart_outunit ) Soil2gw
        ...
        WRITE ( Restart_outunit ) Infil_tot
        ...
        WRITE ( Restart_outunit ) Soil_type
        WRITE ( Restart_outunit ) Slowcoef_lin
        WRITE ( Restart_outunit ) Slowcoef_sq
        WRITE ( Restart_outunit ) Ssr2gw_rate
        WRITE ( Restart_outunit ) Ssr2gw_exp
        WRITE ( Restart_outunit ) Soil2gw_max
        WRITE ( Restart_outunit ) Swale_limit
        WRITE ( Restart_outunit ) Dunnian_flow
        IF ( Pref_flag==1 ) THEN
          WRITE ( Restart_outunit ) Basin_prefflow, Basin_pref_flow_in, Basin_pref_stor, Basin_gvr2pfr, Basin_dunnian_pfr
          WRITE ( Restart_outunit ) Pref_flow_thrsh
          WRITE ( Restart_outunit ) Pref_flow_flag
          WRITE ( Restart_outunit ) Pref_flow_stor
          WRITE ( Restart_outunit ) Pref_flow
          WRITE ( Restart_outunit ) Pref_flow_infil
          WRITE ( Restart_outunit ) Pref_flow_den
          WRITE ( Restart_outunit ) Pref_flow_max
          WRITE ( Restart_outunit ) Gvr2pfr
          WRITE ( Restart_outunit ) Fastcoef_lin
          WRITE ( Restart_outunit ) Fastcoef_sq
        ENDIF
        IF ( Model==0 ) THEN
          WRITE ( Restart_outunit ) Hru_gvr_count
          WRITE ( Restart_outunit ) Hru_gvr_index
          WRITE ( Restart_outunit ) Hrucheck
          WRITE ( Restart_outunit ) Replenish_frac
        ...
        ENDIF
        IF ( Cascade_flag==1 ) THEN
          WRITE ( Restart_outunit ) Basin_dndunnianflow, Basin_dninterflow, Basin_dncascadeflow, Basin_szfarflow
          WRITE ( Restart_outunit ) Upslope_interflow
          WRITE ( Restart_outunit ) Upslope_dunnianflow
          WRITE ( Restart_outunit ) Hru_sz_cascadeflow
          IF ( Nlake>0 ) THEN
            WRITE ( Restart_outunit ) Lakein_sz
            WRITE ( Restart_outunit ) Basin_lakeinsz, Basin_lakeprecip
          ENDIF
        ENDIF
        IF ( Nlake>0 ) WRITE ( Restart_outunit ) Lake_evap_adj
      ELSE
        READ ( Restart_inunit ) module_name
        CALL check_restart(MODNAME, module_name)
        READ ( Restart_inunit ) Pref_flag, Basin_gwin, Basin_sz2gw, Basin_ssr2gw, Basin_interflow_max, Basin_sm2gvr_max, &
     &         Basin_soil_rechr, Basin_dunnian_gvr, Basin_recharge, Basin_ssin, Basin_sm2gvr, Basin_capwaterin, Basin_dunnian, &
     &         Basin_slowflow, Basin_slstor, Basin_cap_infil_max, Basin_cap_up_max, Basin_capillary_wb, &
     &         Basin_gravity_wb, Basin_soilzone_wb, Basin_soil_moist_tot, Basin_gvr2sm, Max_gvrs
        READ ( Restart_inunit ) Soil2gw
        ...
        READ ( Restart_inunit ) Soil_type
        READ ( Restart_inunit ) Slowcoef_lin
        READ ( Restart_inunit ) Slowcoef_sq
        READ ( Restart_inunit ) Ssr2gw_rate
        READ ( Restart_inunit ) Ssr2gw_exp
        READ ( Restart_inunit ) Soil2gw_max
        READ ( Restart_inunit ) Swale_limit
        READ ( Restart_inunit ) Dunnian_flow
        IF ( Pref_flag==1 ) THEN
          READ ( Restart_inunit ) Basin_prefflow, Basin_pref_flow_in, Basin_pref_stor, Basin_gvr2pfr, Basin_dunnian_pfr
          READ ( Restart_inunit ) Pref_flow_thrsh
          READ ( Restart_inunit ) Pref_flow_flag
          READ ( Restart_inunit ) Pref_flow_stor
          READ ( Restart_inunit ) Pref_flow
          READ ( Restart_inunit ) Pref_flow_infil
          READ ( Restart_inunit ) Pref_flow_den
          READ ( Restart_inunit ) Pref_flow_max
          READ ( Restart_inunit ) Gvr2pfr
          READ ( Restart_inunit ) Fastcoef_lin
          READ ( Restart_inunit ) Fastcoef_sq
        ENDIF
        IF ( Model==0 ) THEN
          READ ( Restart_inunit ) Hru_gvr_count
          ALLOCATE ( Hru_gvr_index(Max_gvrs, Nhru) )
          READ ( Restart_inunit ) Hru_gvr_index
          READ ( Restart_inunit ) Hrucheck
          READ ( Restart_inunit ) Replenish_frac
        ...
        ENDIF
        IF ( Cascade_flag==1 ) THEN
          READ ( Restart_inunit ) Basin_dndunnianflow, Basin_dninterflow, Basin_dncascadeflow, Basin_szfarflow
          READ ( Restart_inunit ) Upslope_interflow
          READ ( Restart_inunit ) Upslope_dunnianflow
          READ ( Restart_inunit ) Hru_sz_cascadeflow
          IF ( Nlake>0 ) THEN
            READ ( Restart_inunit ) Lakein_sz
            READ ( Restart_inunit ) Basin_lakeinsz, Basin_lakeprecip
          ENDIF
        ENDIF
        IF ( Nlake>0 ) READ ( Restart_inunit ) Lake_evap_adj
      ENDIF
