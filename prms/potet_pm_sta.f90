...
!******Get parameters
      ELSEIF ( Process_flag==INIT ) THEN
        Vp_sat = 0.0D0
        IF ( getparam(MODNAME, 'pm_n_coef', 12, 'real', Pm_n_coef)/=0 ) CALL read_error(2, 'pm_n_coef')
        IF ( getparam(MODNAME, 'pm_d_coef', 12, 'real', Pm_d_coef)/=0 ) CALL read_error(2, 'pm_d_coef')
        IF ( getparam(MODNAME, 'crop_coef', 12, 'real', Crop_coef)/=0 ) CALL read_error(2, 'crop_coef')
