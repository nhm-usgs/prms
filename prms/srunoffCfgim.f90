...
!   Declared Parameters for Frozen Ground
      REAL, SAVE :: Cfgi_thrshld, Cfgi_decay, Soil_depth(:), Soil_den(:), Porosity_hru(:)
!   Declared Variables for Frozen Ground
      REAL, SAVE, ALLOCATABLE :: Cfgi(:), Cfgi_prev(:), Frz_depth(:), Thaw_depth(:)
...
!***********************************************************************
     ...
     &    Tcal, Tcal_nosnow, Glacrcov_area, Prev_ann_tempc
      IMPLICIT NONE
      INTRINSIC SNGL, DBLE
...
! Local Variables
      ...
      REAL :: cfgi_kg, depthg_cm, soil_cond, latent_soil, nice, ice_cond ! frozen ground
      ...
      REAL :: glcrmltb, temp, temp2, trad, emiss, emisl ! glaciers
...
!     that includes soil_moist for fraction of hru_area that is dry bank
     ...
     &         PRINT *, 'srunoff lake ERROR', Infil(i), Sroff(i), Imperv_stor(i), Imperv_evap(i), i
        ...
        IF ( Frozen_flag==1 ) THEN
          ...
          ! energy available to snowpack proxy temperature
          ...
          trad = ( Snowcov_area(i)*Tcal(i)/(emiss*0.585E-7) )**0.25 - 273.15 !energy that is available to heat land under snow
          trad = trad+( nosnow_area*Tcal_nosnow(i)/(emisl*0.585E-7))**0.25 - 273.15 !energy that is available to heat land without snow
          ...
          depth_cm = SNGL(Pk_depth(i)/Snowcov_area(i))*2.54
          ! depth ground cover only, from Follum et al, 2018, but was in Vermont
          If (Cov_type(i)==0) depthg_cm = 0.0 !bare soil (rock, may be mostly impervious already)
...
! If above the threshold to be frozen
          IF ( Cfgi(i)>=Cfgi_thrshld ) THEN
            ...
            latent_soil = 334.0*Soil_den(i)*omega/1.e6 ! J/m^3, latent heat of fusion of water =  334 J/g , density in g/cm3 */100
            ...
            lambda = ( 1.0 + 0.147*fusion_param_mu*((beta*thermal_ratio_alp)**2.0)
            ...
            omega = Soil_moist(i) / (Soil_depth(i)*Soil_den(i)) !is Soil_moist correct?
            ...
            frz_height = lambda*( (48.0*( Cfgi(i) - Cfgi_thrshld )*mean_cond/lat_soil)**0.5 )/INCH2M
...
!***********************************************************************
     ...
     &    Carea_max, Carea_min, Carea_dif, Contrib_fraction, Thaw_frac
...
!***********************************************************************
     ...
     &                            Basin_dprst_volop, Basin_dprst_volcl, Basin_contrib_fraction
        ...
        IF ( Frozen_flag==1 ) THEN
          ...
          WRITE ( Restart_outunit ) Frz_depth, Thaw_depth
        ENDIF
      ELSE
     ...
     &                          Basin_dprst_volop, Basin_dprst_volcl, Basin_contrib_fraction
        ...
        IF ( Frozen_flag==1 ) THEN ! could be problem for restart
          ...
          READ ( Restart_inunit ) Frz_depth, Thaw_depth
        ENDIF
      ENDIF
