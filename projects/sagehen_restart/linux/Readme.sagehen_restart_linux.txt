...
Control and shell script files are provided in the 'windows' subdirectory to
run the problem in three model modes. 

1. GSFLOW mode: The shell script file for this series of restart simulations 
...
has been reset to '1983,9,1,0,0,0' in the shell script file; several other
input control parameters also are reset for this and each of the subsequent
runs.
...
'bin' directory named 'CSV_merge' merges all of the csv output
...
2. PRMS mode: The shell script file for this simulation is 'gsflow_prms.sh' and
the control file is 'gsflow_prms.control.' Note that control parameter
'model_mode' is reset from a value of GSFLOW5 to PRMS5 in the shell script file.
This simulation extends from October 1, 1980, through January 30, 1984. Output
for the simulation is provided in the '2_PRMS_model' subdirectory under the
'output-test' subdirectory.

3. MODFLOW mode: The shell script file for this simulation is
'gsflow_modflow.sh' and the control file is 'gsflow.modflow.control.' Note
that although the value of 'start_time' is set to October 1, 1980, and the
value of 'end_time' to January 30, 1984, in the control file, the simulation
actually extends through September 30, 1996, because the number of time steps
specified for the second stress period of this simulation in the discretization
file is 5,844 days; this demonstrates that for a MODFLOW-only simulation, the
discretization file time information takes precedence. Output for the simulation
is provided in the '3_MODFLOW_model' subdirectory under the 'output-test'
subdirectory.
