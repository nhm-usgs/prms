...
module UTILS_PRMS
  ...
  contains
    ...
    function get_first_time(iunit, datetime) result(res)
    ...
    function get_next_time(iunit) result(res)
      ! use prms_constants, only: YEAR, MONTH, DAY
      implicit none
    ...
    !***********************************************************************
    subroutine open_dyn_param_file(nhru, Iunit, Iret, Fname, Paramname)
      use iso_fortran_env, only: output_unit
      implicit none
    ...
    !***********************************************************************
    ! Updated values are read in, parameter values are are updated or old
    !***********************************************************************
    module subroutine update_parameter_i32(ctl_data, model_time, dyn_output_unit, dyn_values, param_name, param)
      use prms_constants, only: YEAR, MONTH, DAY
      ! use Control_class, only: Control
