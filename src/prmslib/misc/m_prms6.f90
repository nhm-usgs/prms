module m_prms6
    ...
    character(len=*), parameter :: VERSION = 'prms6.f90 2017-09-29 13:51:00Z'
  ...
  contains
    ...
    !**********************************************************************
    subroutine module_doc(ctl_data, param_data) ! , var_data)
      ...
      type(Parameters), intent(inout) :: param_data
      ! type(variables_arr_t), intent(inout) :: var_data  ! should be intent(in)

      ! Local variable
      integer(i32) :: test

      ! TODO: modify to work with new approach
      !**********************************************************************
    ...
    !***********************************************************************
    ! subroutine call_modules_restart(ctl_data, In_out)
    ...
    !***********************************************************************
    subroutine computation_order(Arg, ctl_data, param_data)  ! , var_data)
      use prms_constants, only: EQULS
      ...
      use PRMS_MODULE, only: Process, Elapsed_time, Elapsed_time_start, &
                             ...
                             ! Et_module, Solrad_module, Transp_module, &   ! NhruOutON_OFF, BasinOutON_OFF
                             Var_save_file
      ! use PRMS_BASIN, only: basin, Hemisphere, Basin_area_inv
      use PRMS_OBS, only: obs
      ! use PRMS_POTET_JH, only: potet_jh
      ...
      type(Control), intent(inout) :: ctl_data
      type(Parameters), intent(inout) :: param_data
      ...
      ! if (Process == 'setdims') then
      !   call DATE_AND_TIME(VALUES=Elapsed_time_start)
      !   Execution_time_start = Elapsed_time_start(5) * 3600 + &
      !                          Elapsed_time_start(6) * 60 + &
      ...
      if (ctl_data%nhruOutON_OFF%values(1) > 0) call nhru_summary(ctl_data, param_data)
      if (ctl_data%basinOutON_OFF%values(1) == 1) call basin_summary(ctl_data, var_data)
