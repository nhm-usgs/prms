submodule (PRMS_ROUTING) sm_routing
  contains
    module function constructor_Routing(ctl_data, param_data, model_basin, model_time) result(this)
      ...
      associate(nhru => ctl_data%nhru%value, &
        ...
        !   allocate ( this%c1(nsegment), this%c2(nsegment), this%c0(nsegment), this%ts(nsegment), this%ts_i(nsegment) )
        ! endif
        ...
        if (strmflow_module(1)%s == 'muskingum' .or. strmflow_module(1)%s == 'muskingum_lake') then
