...
contains
  ...
  module function constructor_Climate_HRU(ctl_data) result(this)
    ...
    ! Control
    ! nhru, cbh_binary_flag, print_debug, start_time
  ...
  module subroutine run_Climate_HRU(this, ctl_data, param_data, model_time, model_basin, model_potet, model_temp, climate)
    ...
    ! Control
    ! nhru, nmonths,
    ! used directly: precip_module, temp_module,

    ! Parameter
    ! hru_area
    ! used directly: tmax_cbh_adj, tmin_cbh_adj, rain_cbh_adj, snow_cbh_adj,
    !                adjmix_rain,
    ...
    associate(curr_month => model_time%Nowmonth, &
      ...
      ! *****************
      ! if (ctl_data%temp_module%values(1)%s == 'climate_hru') then
      !   read(this%tmax_funit, *, IOSTAT=ios) yr, mo, dy, hr, mn, sec, (tmaxf(jj), jj=1, nhru)
      !   read(this%tmin_funit, *, IOSTAT=ios) yr, mo, dy, hr, mn, sec, (tminf(jj), jj=1, nhru)
      !
      ...
      ! *****************
      if (ctl_data%precip_module%values(1)%s == 'climate_hru') then
        read(this%precip_funit, *, IOSTAT=ios) yr, mo, dy, hr, mn, sec, (hru_ppt(jj), jj=1, nhru)
        ...
        climate%newsnow = 0
        climate%prmx = 0.0
        climate%hru_rain = 0.0
        climate%hru_snow = 0.0

        call climate%set_precipitation_form(ctl_data, param_data, model_basin, model_temp, &
